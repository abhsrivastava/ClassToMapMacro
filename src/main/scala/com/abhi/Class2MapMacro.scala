   ...
   inline def apply(defn: Any) : Any = meta {
      defn match {
         case cls: Defn.Class =>
            ...
            } yield q"(${param.name.syntax}, ${Term.Name(param.name.value)})"
            val t : Term = q"_root_.scala.collection.Map[String, Any](..$list)"
            val func = q"def toMap() : _root_.scala.collection.Map[String, Any] = $t"
